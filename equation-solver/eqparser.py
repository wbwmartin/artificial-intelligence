...
class Node:
    ...
    def __str__(self):
        ...
        if (self.type == "EQUALS") or (self.type == "BINARYOP"):
            if self.leaf is '^': 
                return self.fancyRep(self.children[0]) + pretty_printing_colors[self.type] + self.leaf + bcolors.ENDC + self.fancyRep(self.children[1])
        ...
        if (self.type == "UNARYOP"):
            return  pretty_printing_colors[self.type] + self.leaf + bcolors.ENDC + self.fancyRep(self.children)
    ...
    def fancyRep(self, child):
        global prec, opdict
        if child.type == 'BINARYOP' and prec[child.leaf] < prec[self.leaf]:
            return '(' + str(child) + ')'
